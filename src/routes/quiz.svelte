<script>
  ...
  // 문제를 풀고 있는 중인지를 저장(false: 게임 설명 모드, true: 문제 풀기 모드)
  let isRunning = false;
...
{#if !isRunning}
  ...
  <p>
    ...
    tip: 빨리 맞출수록 점수가 많아요!! 늦게 맞춰도 점수는 있으니 진행바에 쫄지
    마세요.<br /><br />
...
{:else}
  ...
  {#if idx % 2 == 0}
    <!-- [색깔 찾기]-->
    <div
      style="color:{current.question.color};background-color:white;"
      class="question"
    >
    ...
    {#each current.choice as item}
      <button
        ...
        on:click={() => {
          $score =
            $score + (current.question.text == item.color ? 10 + bonus : 0);
          next();
  ...
  {:else}
    <!-- [텍스트 찾기]-->
    <div
      style="color:white;background-color:{current.question.color};"
      class="question"
    >
      {kolor[current.question.text]}
    </div>
    ...
    {#each current.choice as item}
      <button
        ...
        on:click={() => {
          $score =
            $score + (current.question.color == item.text ? 10 + bonus : 0);
          next();
        }}>{kolor[item.text]}</button
      >
    {/each}
...
<p style="text-align:center;">
  {#if isRunning}
    <button
      style="width:45%;height:60px;font-weight:bold;font-size:30px"
